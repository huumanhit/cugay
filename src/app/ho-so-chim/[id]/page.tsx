...
import TrustCard from "@/components/birds/TrustCard";
      ...
      <div className="grid lg:grid-cols-[280px_1fr_260px] gap-6">
        ...
        <div className="space-y-3">
          ...
          <div className="relative aspect-square rounded-2xl overflow-hidden bg-accent">
            ...
            />
            {bird.verified && (
              <div className="absolute top-3 left-3 flex items-center gap-1 bg-blue-500 text-white text-xs font-semibold px-2.5 py-1 rounded-full">
          ...
          <div className="grid grid-cols-4 gap-2">
            {bird.gallery.map((img, i) => (
              ...
              >
                <Image src={img} alt="" fill className="object-cover" sizes="70px" />
              </button>
            ))}
          </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border font-medium text-sm transition-all ${
                  liked ? "bg-red-50 text-red-500 border-red-200" : "bg-white text-muted border-border hover:border-red-300"
                }`}
              >
                <Heart className={`w-4 h-4 ${liked ? "fill-red-500" : ""}`} />
                {liked ? "Đã yêu thích" : "Yêu thích"}
              </button>
              <button className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border bg-white text-muted hover:border-primary-300 font-medium text-sm transition-all">
          ...
          <div className="bg-white rounded-2xl border border-border shadow-card p-5 mb-4">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {(bird.rank ?? 0) <= 3 && bird.rank && (
                    <Badge variant="pro">Top {bird.rank} Quốc gia</Badge>
                  )}
                </div>
              ...
              {bird.price && bird.forSale && (
                <div className="text-right">
                  <p className="text-2xl font-bold text-primary-600">
                    {(bird.price / 1_000_000).toFixed(0)} tr
                  </p>
            ...
            <div className="flex gap-4 mt-3 pt-3 border-t border-border">
              <div className="flex items-center gap-1 text-sm text-muted">
                ...
                {formatNumber(bird.views)} lượt xem
              </div>
              <div className="flex items-center gap-1 text-sm text-muted">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 text-xs text-muted">
                    <MapPin className="w-3 h-3" />{bird.province}
                  </div>
                </div>
                <Link href="#" className="ml-auto text-xs text-primary-600 font-semibold hover:underline">Xem trang</Link>
              </div>
            </div>
          </div>
          ...
          {bird.achievements.length > 0 && (
            <div className="bg-white rounded-2xl border border-border shadow-card p-5 mb-4">
              <h2 className="font-semibold text-text-primary mb-3 flex items-center gap-2">
                ...
                Thành tích ({bird.achievements.length})
              </h2>
              <div className="space-y-2">
            ...
            <Tabs defaultValue="overview">
              ...
              <TabsContent value="overview">
                <div className="prose prose-sm text-muted">
                  <p className="leading-relaxed">{bird.description}</p>
                  <p className="mt-3 leading-relaxed">
                    {bird.name} được nuôi dưỡng theo tiêu chuẩn kỹ thuật cao, với chế độ dinh dưỡng và luyện tập khoa học.
                    Chim có bộ lông {bird.color.toLowerCase()}, cân nặng lý tưởng {bird.weight ?? "phù hợp"} và giọng {bird.voice} đặc trưng vùng {bird.origin}.
                  </p>
                </div>
              </TabsContent>
              ...
              <TabsContent value="timeline">
                {events.length > 0 ? (
                  ...
                  <p className="text-sm text-muted text-center py-8">Chưa có nhật ký nào được ghi lại.</p>
                )}
              </TabsContent>
              ...
              <TabsContent value="videos">
                <div className="grid grid-cols-2 gap-3 pt-2">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="relative aspect-video bg-accent rounded-xl overflow-hidden group cursor-pointer">
                      <Image src={bird.gallery[i % bird.gallery.length]} alt="" fill className="object-cover" sizes="200px" />
                ...
                <div className="grid grid-cols-3 gap-2 pt-2">
                  {bird.gallery.map((img, i) => (
                    <div key={i} className="relative aspect-square rounded-xl overflow-hidden">
                      <Image src={img} alt="" fill className="object-cover hover:scale-105 transition-transform cursor-pointer" sizes="120px" />
                    </div>
                  ))}
                </div>
              </TabsContent>
              ...
              <TabsContent value="genealogy">
                <div className="text-center py-8 text-muted">
                  ...
                  <p className="text-sm mt-1">Thông tin phả hệ đang được cập nhật.</p>
                </div>
              </TabsContent>
              ...
              <TabsContent value="transactions">
                <div className="text-center py-8 text-muted">
                  <p className="text-4xl mb-3">📋</p>
                  <p className="font-medium text-text-primary">Lịch sử giao dịch</p>
                  <p className="text-sm mt-1">Chưa có giao dịch nào được ghi nhận.</p>
                </div>
              </TabsContent>
            </Tabs>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-border shadow-card p-5">
            ...
            <div className="bg-accent rounded-xl p-4 text-center">
              ...
              <p className="text-[10px] text-muted mt-1">Mã QR hồ sơ chính thức</p>
            </div>
          </div>
