...
import { Heart, Share2, Phone, CheckCircle2, Award, MapPin, Bird, Eye, Star } from "lucide-react";
...
import type { Bird as BirdType, TimelineEvent } from "@/types";
...
export default function BirdDetailClient({ bird, events, similar }: Props) {
  const [activeImage, setActiveImage] = useState(0);
  const [liked, setLiked] = useState(false);

  const gallery = bird.gallery.length > 0 ? bird.gallery : [bird.image];
    ...
    <div className="grid lg:grid-cols-[280px_1fr_260px] gap-6">
      ...
      <div className="space-y-3">
        <div className="relative aspect-square rounded-2xl overflow-hidden bg-accent">
          <BirdImage src={gallery[activeImage] ?? bird.image} alt={bird.name} fill />
          {bird.verified && (
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border bg-white text-muted font-medium text-sm hover:border-primary-300 transition-all">
      ...
      <div>
        <div className="bg-white rounded-2xl border border-border shadow-card p-5 mb-4">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {bird.rank && bird.rank <= 3 && <Badge variant="pro">Top {bird.rank} Quốc gia</Badge>}
              </div>
          ...
          {bird.description && <p className="text-sm text-muted leading-relaxed">{bird.description}</p>}
        ...
        <div className="bg-white rounded-2xl border border-border shadow-card p-5">
          <Tabs defaultValue="overview">
            ...
            <TabsContent value="overview">
              <p className="text-sm text-muted leading-relaxed mt-2">{bird.description || "Chưa có mô tả."}</p>
            </TabsContent>
        ...
        <div className="bg-white rounded-2xl border border-border shadow-card p-5 space-y-4">
          ...
          <div className="pt-2 border-t border-border">
            ...
            <div className="w-full bg-accent rounded-full h-2">
              <div className="bg-gradient-to-r from-primary-400 to-primary-600 h-2 rounded-full" style={{ width: `${Math.min(100, ((bird.score ?? 0) / 10000) * 100)}%` }} />
            </div>
          </div>
        </div>
